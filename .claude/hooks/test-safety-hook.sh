...
test_should_allow "git add .env.example" "Allow staging .env.example (meant to be committed)"
test_should_allow "rm -rf .context/scratch" "Allow cleaning .context (ephemeral workspace)"
test_should_allow "psql -h localhost -c 'DELETE FROM products'" "Allow DELETE on products locally (prod must not substring-match)"
